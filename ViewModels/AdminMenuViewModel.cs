...
namespace ClinicaApp.ViewModels
{
    public partial class AdminMenuViewModel : BaseViewModel
    {
        ...
        // ✅ NUEVO: Comando para Punto 2
        [RelayCommand]
        private async Task IrAConsultaMedico()
        {
            try
            {
                System.Diagnostics.Debug.WriteLine("[ADMIN MENU] Navegando a consulta médicos");
                await Shell.Current.GoToAsync("//MedicoConsultaPage");
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[ADMIN MENU] Error navegación consulta: {ex.Message}");
            }
        }
        ...
        private async Task IrACreacionCita()
        {
            try
            {
                System.Diagnostics.Debug.WriteLine("[ADMIN MENU] Navegando a creación cita");
                await Shell.Current.GoToAsync("//CitaCreacionPage");
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[ADMIN MENU] Error navegación cita: {ex.Message}");
            }
        }
